...
class Client(TenantMixin):
    ...
    # Login identity: `slug` is the subdomain (greenfield-high.yukicares.cloud);
...
class WhatsAppSupportCode(models.Model):
    """A short, verifiable code that ties a WhatsApp support chat to a real,
    logged-in customer.

    Issued by an authenticated in-app request and stored in the PUBLIC schema so
    a platform support agent can verify any school's code from one console — the
    customer proves they're genuine simply by having generated the code while
    ...
    school_code = models.CharField(max_length=32, blank=True)  # human school code (GHS-3000)
    user_name = models.CharField(max_length=200, blank=True)
    user_email = models.EmailField(blank=True)
    user_phone = models.CharField(max_length=32, blank=True)  # registered phone, for cross-check
    ...
    def __str__(self) -> str:
        return f"{self.code} · {self.school_name or self.schema_name}"
